...
import { JUNGLE_ZONES } from '@/types/jungle';
...
export const JungleBackground: React.FC<JungleBackgroundProps> = ({ currentDistance, className = '' }) => {
  // 현재 거리에 맞는 구역 찾기
  const currentZone = JUNGLE_ZONES.find(
    (zone) => currentDistance >= zone.distance_min && currentDistance <= zone.distance_max
  ) || JUNGLE_ZONES[0];

  // 구역 내에서의 진행도 계산 (0 ~ 1)
  const zoneProgress = Math.min(
    1,
    (currentDistance - currentZone.distance_min) / (currentZone.distance_max - currentZone.distance_min)
  );
